...
argument-hint: "<ID zadania, np. F1-08> albo puste = następne wolne"
---
<!-- WYGENEROWANE przez .agent-os/scripts/sync-adapters.mjs: nie edytuj ręcznie -->
Rozpoczynasz pracę jako **claude** w Agent OS projektu Vocaly. Argument: `$ARGUMENTS`.
...
2. Uruchom `node .agent-os/scripts/task.mjs start <ID> --agent claude`.
