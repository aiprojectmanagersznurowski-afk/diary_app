...
Kończysz sesję jako **gemini**. Następna sesja może być prowadzona przez innego agenta, który nie zna tej rozmowy: wszystko, co ważne, musi być w repozytorium.
...
4. `node .agent-os/scripts/task.mjs handoff --agent gemini argument podany po /os-handoff ([krótka notatka])` (dopisuje wpis w „Sesje”).
